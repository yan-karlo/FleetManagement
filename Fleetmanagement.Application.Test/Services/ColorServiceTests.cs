...
namespace FleetManagement.Tests.Services
{
    public class ColorServiceTests
    {
        ...
        public async Task RemoveAsync_ShouldRemoveColor()
        {
            ...
            // Act
            await _colorService.RemoveAsync(colorId);

            // Assert
            _mockColorRepository.Verify(repo => repo.RemoveAsync(It.Is<Color>(c => c.Id == colorId)), Times.Once);
        }
        ...
        {
            // Arrange
            var colorDTO = new ColorDTO { Id = 1, Name = "Pink" };
            var colorEntity = new Color(1, "Pink");

            _mockMapper.Setup(mapper => mapper.Map<Color>(colorDTO)).Returns(colorEntity);

            // Act
            await _colorService.UpdateAsync(colorDTO);

            // Assert
            _mockColorRepository.Verify(repo => repo.UpdateAsync(It.Is<Color>(c => c.Id == colorEntity.Id && c.Name == colorEntity.Name)), Times.Once);
        }
    }
}
