...
namespace FleetManagement.Tests.Services
{
    public class VehicleTypeServiceTests
    {
        ...
        public async Task RemoveAsync_ShouldRemoveVehicleType()
        {
            ...
            // Act
            await _vehicleTypeService.RemoveAsync(vehicleTypeId);

            // Assert
            _mockVehicleTypeRepository.Verify(repo => repo.RemoveAsync(It.Is<VehicleType>(v => v.Id == vehicleTypeId)), Times.Once);
        }
        ...
        {
            // Arrange
            var vehicleTypeDTO = new VehicleTypeDTO { Id = 1, Name = "UpdatedName", PassengersCapacity = 8 };
            var vehicleTypeEntity = new VehicleType(1, "UpdatedName", 8);

            _mockMapper.Setup(mapper => mapper.Map<VehicleType>(vehicleTypeDTO)).Returns(vehicleTypeEntity);

            // Act
            await _vehicleTypeService.UpdateAsync(vehicleTypeDTO);

            // Assert
            _mockVehicleTypeRepository.Verify(repo => repo.UpdateAsync(It.Is<VehicleType>(v => v.Id == vehicleTypeEntity.Id && v.Name == vehicleTypeEntity.Name)), Times.Once);
        }
    }
}
