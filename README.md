...
4. **Aplicar as Migrações**:
   ...
   Entre no Container da aplicação para aplicar as migrações no banco de dados:

   ```sh
   docker exec -it fleet-management-web-ui bash
   dotnet ef database update
   ```

   Ou se preferir, depois de subir os containers, esse processo também pode ser feito de pelo Visual Studio:
